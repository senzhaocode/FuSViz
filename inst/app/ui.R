		...
		tabsetPanel(id = "tabset-data",
			tabPanel("SV from RNA-seq",
				...
				fluidRow(
					...
					column(12, shinycssloaders::withSpinner(DT::dataTableOutput("RNAcontents", height = 450), type = 8, size = 0.7)),
				),
				br(),
				fluidRow(
						...
						tabBox(id = "Feature_rna", height = "100%", width = "100%",
							...
							tabPanel("SV distribution across samples",
								fluidRow(class = "rna_hist_sample",
									column(12, shinycssloaders::withSpinner(plotOutput("rna_hist_1", width = "100%", height = 150), type = 8, size = 0.7)),
			...
			tabPanel("SV from DNA-seq",
				...
				fluidRow(
					...
					column(12, shinycssloaders::withSpinner(DT::dataTableOutput("DNAcontents", height = 450), type = 8, size = 0.7))
				),
				br(),
				fluidRow(
					box(title = "Feature summary", status="warning", width=12, height = "100%", collapsible = TRUE, collapsed = FALSE,
						tabBox(id = "Feature_dna", height = "100%", width = "100%",
							...
							tabPanel("SV distribution across samples",
								fluidRow(class = "dna_hist_sample",
									...
									column(12, shinycssloaders::withSpinner(plotOutput("dna_hist_1", width = "100%", height = 190), type = 8, size = 0.7)),
			...
			tabPanel("Mutation profile",
				...
				fluidRow(
					...
					column(12, shinycssloaders::withSpinner(DT::dataTableOutput("DNAMutations", height = 450), type = 8, size = 0.7)) 
				),
				br(),
				fluidRow(
					box(title = "Feature summary", status="warning", width=12, height = "100%", collapsible = TRUE, collapsed = FALSE,
						tabBox(id = "Feature_mut", height = "100%", width = "100%",
							...
							tabPanel("Correlation of mutation and SV (RNA-seq) burden",
								column(12, shinycssloaders::withSpinner(plotOutput("rna_mut_cor", width="100%", height=600, brush=brushOpts(id="rna_brush")), type=8, size=0.7)),
								column(12, h4("Selected samples"), verbatimTextOutput("rna_mut_info"))
							),
							tabPanel("Correlation of mutation and SV (DNA-seq) burden",
								column(12, shinycssloaders::withSpinner(plotOutput("dna_mut_cor", width="100%", height=600, brush=brushOpts(id="dna_brush")), type=8, size=0.7)),
			...
			tabPanel("Drug target info", 
				...
				fluidRow(
					...
					column(12, shinycssloaders::withSpinner(DT::dataTableOutput("drug", height = 600), type = 8, size = 0.7))
				)
			)
		)
	)
				...
				tabBox(id = "circle_plot_tab", height = "100%", width = "100%",
					tabPanel("RNA_SV_circular_plot",
						sidebarLayout(
							sidebarPanel(width = 3,
								...
								conditionalPanel(condition = "input.cirmutcheck == 1",
									selectizeInput("mut_sample", label = div("Mutation sample", style = "display:inline-block; font-size:13px", title = "Limit to plot small variant profile related to the choosen samples", icon("info-circle", style="font-size: 10px")), choices = NULL, multiple = T, options = list(placeholder = 'select', onInitialize = I('function() { this.setValue(""); }')))),
								br(),
							...
							mainPanel(width = 9,
								fluidRow(
									shinycssloaders::withSpinner(BioCircos::BioCircosOutput("circle_1", height=750), type = 8, size = 0.7)
								)
							)
						)
					),
					tabPanel("DNA_SV_circular_plot",
						sidebarLayout(
							sidebarPanel(width = 3,
								...
								conditionalPanel(condition = "input.cirmutcheck_dna == 1", 
									selectInput("mut_type_dna", label = div("Mutation type", style = "display:inline-block; font-size:13px", title = "Limit to plot small variant profile of the choosen mutation types", icon("info-circle", style="font-size: 10px")), choices = c("Frame_Shift_Del", "Frame_Shift_Ins", "Splice_Site", "Translation_Start_Site", 
										...
										"Start_Codon_SNP", "Stop_Codon_Del"), multiple = T)),
								conditionalPanel(condition = "input.cirmutcheck_dna == 1",
									selectizeInput("mut_sample_dna", label = div("Mutation sample", style = "display:inline-block; font-size:13px", title = "Limit to plot small variant profile related to the choosen samples", icon("info-circle", style="font-size: 10px")), choices = NULL, multiple = T, options = list(placeholder = 'select', onInitialize = I('function() { this.setValue(""); }')))),
								br(),
							...
							mainPanel(width = 9,
								fluidRow(
									shinycssloaders::withSpinner(BioCircos::BioCircosOutput("circle_2", height=750), type = 8, size = 0.7)
								)
							)
						)
					)
			...
			sidebarPanel(width = 3,
				...
				tabsetPanel(id = "igv-panel-control",
					tabPanel("DNA_SV",
						...
						hr(tags$style("border-top: 3px solid #000000; color: black; background-color: black")),
						fluidRow(
							column(12, fileInput('cnv_baf_data', label = div(HTML('<p>Upload BAF plot file <a href="https://igv.org/doc/igvjs/#tracks/CNVPytor/" target="_blank">(See the format requirement)</a></p>'),
								style = "display:inline-block;", title = "Compressed VCF file (*.baf.vcf.gz) is used for BAF plot (NOTE: no need for the index file)", icon("info-circle", style="font-size: 9px")), accept = c('.gz'), multiple = T, placeholder = "compressed vcf format")),
							column(12, selectizeInput("bin_size", label = div("Bin_size", style = "display:inline-block; font-size:12px", title = "Bin size, available values: 10000, 100000, 1000000, 10000000 or 100000000", icon("info-circle", style="font-size: 9px")), choices = c(10000, 100000, 1000000, 10000000, 100000000), selected = 1000000, multiple = F)),
							column(12, actionButton("BinCustomize", label = div("Update and refresh the BAF plot track", title = "Click on ‘Update and refresh the BAF plot track’ button and set a customized BAF plot track")))
						)
					),
					tabPanel("RNA_SV",
						fluidRow(
						...
						hr(tags$style("border-top: 3px solid #000000; color: black; background-color: black")),
						fluidRow(
							column(12, fileInput('junction_bed_data', label = div(HTML('<p>Upload splice junction file <a href="https://igv.org/doc/igvjs/#tracks/Splice-Junctions/" target="_blank">(See the format requirement)</a></p>'),
								style = "display:inline-block;", title = "Splice junction file (*.SJ.out.tab.bed.gz) and its index file (*.SJ.out.tab.bed.gz.tbi) for uploading is originally generated by STAR aligner", icon("info-circle", style="font-size: 9px")), accept = c('.gz', '.tbi'), multiple = T, placeholder = "compressed bed format")),
							column(6, numericInput("rna_junction_unique", label = div("Num_unique", style = "display:inline-block; font-size:12px", title = "Junction must be supported by at least this number of uniquely-mapped reads", icon("info-circle", style="font-size: 9px")), value=1, min=1, max=10000)),
							column(6, numericInput("rna_junction_total", label = div("Num_total", style = "display:inline-block; font-size:12px", title = "Junction must be supported by at least this number of uniquely-mapped + multi-mapped reads", icon("info-circle", style="font-size: 9px")), value=1, min=1, max=10000)),
							column(6, numericInput("rna_junction_percet", label = div("Num_percet", style = "display:inline-block; font-size:12px", title = "(uniquely-mapped reads)/(total reads) must be <= this threshold", icon("info-circle", style="font-size: 9px")), value=1, min=0, max=1)),
							column(6, numericInput("rna_junction_overhang", label = div("Num_overhang", style = "display:inline-block; font-size:12px", title = "Mininum spliced alignment overhang in base pairs", icon("info-circle", style="font-size: 9px")), value=1, min=0, max=500)),
							column(12, selectizeInput("rna_junction_color", label = div("Color_by", style = "display:inline-block; font-size:12px", title = "Splice junction color, available values: 'numUniqueReads', 'numReads', 'isAnnotatedJunction', 'strand', 'motif'", icon("info-circle", style="font-size: 9px")), choices = c('numUniqueReads', 'numReads', 'isAnnotatedJunction', 'strand', 'motif'), selected = 'motif', multiple = F)),
							column(12, selectizeInput("rna_junction_thick", label = div("Thickness", style = "display:inline-block; font-size:12px", title = "Splice junction line thickness, available values: 'numUniqueReads', 'numReads', 'isAnnotatedJunction'", icon("info-circle", style="font-size: 9px")), choices = c('numUniqueReads', 'numReads', 'isAnnotatedJunction'), selected = 'numUniqueReads', multiple = F)),
							column(12, selectizeInput("rna_junction_curve", label = div("Curve_height", style = "display:inline-block; font-size:12px", title = "Splice junction curve height, available values: 'random', 'distance', 'thickness'", icon("info-circle", style="font-size: 9px")), choices = c('random', 'distance', 'thickness'), selected = 'distance', multiple = F)),
							column(12, actionButton("JunctionCustomize", label = div("Update and refresh splice junction tracks", title = "Click on ‘Update and refresh current junction tracks’ button and set a customized junction track")))
						)
					),
					tabPanel("Mut",
						fluidRow(
	...
	bodyTwoway <- tabItem(tabName = "fusion", value="fusion_plot", 
		...
		fluidRow(
			box(status="warning", width=12, height = "100%",
				tabBox(id = "fusion_plot_page", height = "100%", width = "100%",
							...
							mainPanel(width = 9,
								fluidRow(
									...
									column(12, div(class="overviewmain", id="overviewmain",
										shinycssloaders::withSpinner(plotOutput("chimerics_down", width = "100%", height = "100%"), type = 8, size = 0.7)
									)),
					...
					tabPanel("Sample_plot", 
						sidebarLayout(
							...
							mainPanel(width = 9,
								fluidRow(
									column(12, shinycssloaders::withSpinner(plotOutput("chimerics2", width = "100%", height = "100%"), type = 8, size = 0.7))
								),
								box(width = NULL, status = "warning",
					...
					tabPanel("Domain_plot", 
						sidebarLayout(
							...
							mainPanel(width = 9,
								fluidRow(
									column(12, div(class="domainA", shinycssloaders::withSpinner(plotOutput("domain_up", height=220), type = 8, size = 0.7))),
									column(12, div(class="fuseline", plotOutput("linking", height=30))),
									column(12, div(class="domainB", shinycssloaders::withSpinner(plotOutput("domain_down", height=220), type = 8, size = 0.7))),
				...
				tabBox(id = "network_setting_tab", height = "100%", width = "100%",
					tabPanel("RNA_SV_network_panel",
						sidebarLayout(
							sidebarPanel(width = 3,
								...
								br(),
								fluidRow(column(12, tags$p(tags$h5("Parameter Setting:", style="margin-left: 0em; text-align: left; font-weight: bold; text-decoration: underline")))),
								fluidRow(
							...
							mainPanel(width = 9,
								tabBox(id = "network_plot_rna_page", height = "100%", width = "100%",
									tabPanel("RNA_SV_network_plot",
										shinycssloaders::withSpinner(visNetwork::visNetworkOutput("network_rna", width = "100%", height = "700px"), type = 8, size = 0.7)
									),
									tabPanel("RNA_SV_network_hub",
										fluidRow(
											# DT and dataTable library implementary
											column(12, shinycssloaders::withSpinner(DT::dataTableOutput("RNAhubs", height = 450), type = 8, size = 0.7))
										)
									)
								)
							)
					...
					tabPanel("DNA_SV_network_panel",
						sidebarLayout(
							...
							mainPanel(width = 9,
								tabBox(id = "network_plot_dna_page", height = "100%", width = "100%",
									tabPanel("DNA_SV_network_plot",
										shinycssloaders::withSpinner(visNetwork::visNetworkOutput("network_dna", width = "100%", height = "700px"), type = 8, size = 0.7)
									),
									tabPanel("DNA_SV_network_hub",
										fluidRow(
											# DT and dataTable library implementary
											column(12, shinycssloaders::withSpinner(DT::dataTableOutput("DNAhubs", height = 450), type = 8, size = 0.7))
										)
									)
								)
							)
	...
	ui <- dashboardPage(
		...
		skin = "black",
		dashboardHeader(title = tags$a(href='https://github.com/senzhaocode/FuSViz',tags$img(src='Visualize/logo.png', height = "65%")), dropdownMenuOutput("messageMenu"), titleWidth = 150),
		manual,
