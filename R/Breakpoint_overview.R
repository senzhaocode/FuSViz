#' Tag breakpoint position in transcripts of partner genes
#'
#' @description Tag breakpoint position in each transcript of partner genes (e.g. at exon boundary/within exon or within intron, exon rank)
#'              for two-way plotting in overview tab-panel (a revision of \emph{gene_trans_ex} function in Breakpoint_persample.R to reduce time complexity)
#'
...
#'
#' @export
gene_trans_ex_overview <- function(breakpoint, object, whole_tx) {
	# For testing: breakpoint = breakpoint_set[1,1]; object=object_A; whole_tx = whole_txdb
	dataset = object$dataset #// a data.frame class with only one row
	start = object$start
	end = object$end
	...
	select_region_f = object$txTr_f #// a data.frame class to collect annotations and structures (5utr-cds-utr3) of transcripts for geneA or geneB 
	JE = c(); #// a data.frame class to collect breakpoint judgement (1, 2, 3 or 4), exon_number, transcript_length and exon_id harboring breakpoint or close to breakpoint
	for ( x in 1:length(dataset[,1])) { #// not important whether x is either character or numeric
		name = dataset[x,]$TXNAME # // transcript_id e.g. name = "ENST00000398958"
		...
		region_f_group = as.data.frame(all_exon_tx_A); 
		colnames(region_f_group)[7] = "exon"
		colnames(region_f_group)[8] = "EXONRANK"
		region_f_group = region_f_group[order(region_f_group$start), , drop=F]
		if ( strand == "+" ) {
		...
		if ( length(region_f_group[,1]) == 1 ) { #// the transcript only has one exon
			if ( breakpoint >= region_f_group[1,]$start && region_f_group[1,]$end >= breakpoint ) {	 #// breakpoint at exon boundary or within exon
				...
				if ( as.character(region_f_group[1,]$strand) == "+" ) {
					prop = (region_f_group[1,]$start - breakpoint + 1)/(region_f_group[1,]$end - breakpoint + 1) #// 0 <= prop <= 1
					flag = region_f_group[1,]; flag[,"Prop"] = prop;
				} else if ( as.character(region_f_group[1,]$strand) == "-" ) {
					prop = (region_f_group[1,]$start - breakpoint + 1)/(region_f_group[1,]$end - breakpoint + 1) #// 0 <= prop <= 1
					flag = region_f_group[1,]; flag[,"Prop"] = prop;
				...
				if ( as.character(region_f_group[1,]$strand) == "+" ) {
					prop = (breakpoint - region_f_group[1,]$end + 1)/(breakpoint - region_f_group[1,]$start + 1) #// 0 <= prop <= 1
					flag = region_f_group[1,]; flag[,"Prop"] = prop;
				} else if ( as.character(region_f_group[1,]$strand) == "-" ) {
					prop = (breakpoint - region_f_group[1,]$end + 1)/(breakpoint - region_f_group[1,]$start + 1) #// 0 <= prop <= 1
					flag = region_f_group[1,]; flag[,"Prop"] = prop;
			...
			for (z in 1:y) {
				if ( z==1 ) { #// if loop at the first exon
					if ( breakpoint < region_f_group[z,]$start ) { #// breakpoint < start_exon_first of transcript
						if ( as.character(region_f_group[z,]$strand) == "+" ) {
							prop = (region_f_group[z,]$start - breakpoint + 1)/(region_f_group[z,]$end - breakpoint + 1) #// 0 <= prop <= 1
				...
				if ( breakpoint >= region_f_group[z,]$start && breakpoint <= region_f_group[z,]$end ) { #// breakpoint at exon boundary or within exon
					...
					break
				} else if ( breakpoint > region_f_group[z,]$end && region_f_group[z+1,]$start > breakpoint ) { #// breakpoint at the boundary or within the last exon
					if ( as.character(region_f_group[z,]$strand) == "+" ) {
				...
				if ( z == y ) { #// loop at the second last exon
					if ( breakpoint >= region_f_group[z+1,]$start && region_f_group[z+1,]$end >= breakpoint ) { #// breakpoint at the boundary or within the last exon
						...
						if ( as.character(region_f_group[z+1,]$strand) == "+" ) {
							prop = (breakpoint - region_f_group[z+1,]$end + 1)/(breakpoint - region_f_group[z+1,]$start + 1) #// 0 <= prop <= 1
							flag = region_f_group[z+1,]; flag[,"Prop"] = prop
						} else if ( as.character(region_f_group[z+1,]$strand) == "-" ) {
							prop = (breakpoint - region_f_group[z+1,]$end + 1)/(breakpoint - region_f_group[z+1,]$start + 1) #// 0 <= prop <= 1
							flag = region_f_group[z+1,]; flag[,"Prop"] = prop
		...
		if ( !is.null(flag) ) { 
			...
			JE = rbind(JE, tmp, deparse.level=0); #// extend row of JE data.frame (including Judge, flag, Exon_num....), although only one row available	
		} else {
	...
	#// 'transcript' is a data.frame class (only one row) used for evaluating breakpoint pos in the given transcript, and 'select_region_f' is a data.frame class used for plotTracks
