...
get_annotation_db <- function(ensemblId, txdb_ref, grTrack_ref) {
	...
	dataset = tryCatch(as.data.frame(txdb_ref[[ensemblId]]), error = function(e) {});

	if ( length(rownames(dataset)) == 0 ) { return(NULL); }
	dataset$tx_id = NULL;	dataset$width = NULL;
	colnames(dataset) = c("TXCHROM", "TXSTART", "TXEND", "TXSTRAND", "TXNAME");
	dataset$TXCHROM=as.character(dataset$TXCHROM);
	dataset$TXSTRAND=as.character(dataset$TXSTRAND);
	dataset$GENEID = ensemblId;
	dataset = dataset[, c("GENEID", "TXNAME", "TXCHROM", "TXSTRAND", "TXSTART", "TXEND")]
